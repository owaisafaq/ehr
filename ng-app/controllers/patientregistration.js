...
AppEHR.controller('patientRegistrationController', ['$rootScope', '$scope', '$window', 'Countries', 'States', 'GetLocalGovermentArea', 'City', 'DropDownData', 'PatientInformation', 'fileUpload', '$location', '$filter', 'Upload', '$timeout', 'PatientRegistrationAddress', 'PatientRegistrationKin', 'PatientRegistrationEmployer', '$routeParams', 'GetPatientAllData', 'PatienPlanSaveData', '$compile', function ($rootScope, $scope, $window, Countries, States, GetLocalGovermentArea, City, DropDownData, PatientInformation, fileUpload, $location, $filter, Upload, $timeout, PatientRegistrationAddress, PatientRegistrationKin, PatientRegistrationEmployer, $routeParams, GetPatientAllData, PatienPlanSaveData, $compile) {
        ...
        $scope.modal_nhis_Data = function (MI) {
            ...
            $('.dependant_list > div').each(function () {
                var id = $(this).children('.chip').data('id')
                var chip_name = $(this).children('.chip').text();
                depedants_values.push({dependent_id: id, relationship: $(this).find('select[name=dependant_relationship]').val()})
...
//            if (check_val == "principal") {
//                if ($('#nhis .dependant_list .chip').length > 0) {
//                    $rootScope.valid_relationship = true;
//                }
//            }
...
//                for(var i = 0; i == $scope.dataToBeAdded.dependents.lenght; i++)
                $scope.dataToBeAdded.dependents = depedants_values
...
//                    ng-options="relationships.name for relationships in  track by relationships.id"

                    $('#nhis .dependant_list').append($compile('<div class="col-lg-12 no-padding"><div class="chip" data-id="' + id_chip + '">' + $('#s2id_get_val_dependant_nhis .select2-chosen').html() + '<i class="md-close"></i></div><div class="col-lg-3"><select ng-class="{true : \'error\'}[checkValidate_nhis && form_modal_1.dependant_relationship.$invalid]" required name="dependant_relationship" class="form-control" ng-model="MI.dependant_relationship" placeholder="Select Relationship"><option value=""></option><option ng-repeat="relationships in dropDownData.relationships" value="{{relationships.id}}">{{relationships.name}}</option></select><span class="help-block PIValid alignError" ng-show="checkValidate_nhis && form_modal_1.dependant_relationship.$error.required">Required</span></div></div>')($scope));
...
//                    $('#s2id_get_val_dependant_nhis').addClass('disable-after-1');
                    $rootScope.valid_relationship = true;
                    console.log($rootScope.valid_relationship)
                }
            }
            else {
                $rootScope.valid_relationship = false;
...
//                };
        ...
        $scope.savePlanData = function () {
            ...
            if ($scope.PP.checkoutpatient == 1)
            {
                ...
                PatienPlanSaveData.save($scope.dataToBeAdded, PlanDatasuccessMessage, PlanDataFailure)
            }
