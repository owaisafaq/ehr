...
AppEHR.controller('radiology', ['$scope', '$rootScope', 'GetAllLabOrders', '$window', '$routeParams','getLabOrderInfo','cancelLabOrder', '$timeout', '$location', 'GetAllPatients', 'DropDownData', 'GetLabTests','addOrder', 'LabOrdersByPatient', 'GetAllRadiologyLabs', function ($scope, $rootScope, GetAllLabOrders, $window, $routeParams,getLabOrderInfo,cancelLabOrder, $timeout, $location, GetAllPatients, DropDownData, GetLabTests, addOrder, LabOrdersByPatient, GetAllRadiologyLabs) {
	$rootScope.pageTitle = "EHR - Lab Order Listing";
    $scope.action = "";
    $rootScope.loader = "show";
    $scope.Order = {};
    $scope.Order.selected_lab = 2;
    ...
    function radiologyGetorerdesucdcess(res){
        console.log(res, "all radio", $routeParams.patientID, $routeParams.encounterID);
        $rootScope.loader = "hide";
        $scope.raidoLabsORders = res.data;
        $scope.raidoCount = res.count;
    }
    function radiologyGetorerdeFailure(error){
        console.log(error);
    }
    ...
    $scope.createOrder = function (Order) { // creating order
        ...
        console.log($scope.Order.patient_id, $scope.Order.selected_lab, $scope.lab_tests, $scope.Order.clinical_information, $scope.Order.diagnosis, $routeParams.encounterID);
    ...
    $scope.paginationNext = function(pageSize, curPage){
        ...
        GetAllRadiologyLabs.save({token: $window.sessionStorage.token, patient_id: $routeParams.patientID, visit_id: $routeParams.encounterID, offset: (pageSize * curPage), limit: $scope.itemsPerPage}, radiologyGetorerdesucdcess, radiologyGetorerdeFailure);
    }
